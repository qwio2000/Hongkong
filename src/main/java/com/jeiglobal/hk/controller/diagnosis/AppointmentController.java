...
public class AppointmentController {
	...
	public String appointmentList(Model model, @ModelAttribute LoginInfo loginInfo) {
		...
		List<AppointmentDto.AppointmentSerch> appointmentSerch = appointmentService.getAppointmentSearch("", loginInfo.getJisaCD(), loginInfo.getDeptCD(), 1, pageSize);
	...
	@ResponseBody
	public Map<String, Object> getAppointmentSerchJson(Model model,	String name, @ModelAttribute LoginInfo loginInfo){
		log.debug("get Appointment Serch : {}", name);
	
		List<AppointmentDto.AppointmentSerchJson> appointmentSerchJson = appointmentService.getAppointmentSearchJson(name, loginInfo.getJisaCD(), loginInfo.getDeptCD());
		Map<String, Object> map = new HashMap<>();
		map.put("result", appointmentSerchJson);
		return map;
	}
